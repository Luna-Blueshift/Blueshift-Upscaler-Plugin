...
void UBlueshiftIQManager::ResetUpscalerSettings()
{
...
void UBlueshiftIQManager::UpdateUpscalerEnabled()
{
	check(IsInGameThread());
	UpdateDLSSEnabled();
	UpdateFSREnabled();
...
void UBlueshiftIQManager::ApplyUpscalerSettings()
{
	check(IsInGameThread());
...
bool UBlueshiftIQManager::SetDesiredUpscaler(const EBlueshiftUpscalerMode InUpscalerMode)
{
	// Validate that the upscaler is actually supported first
	if (IsUpscalerSupported(InUpscalerMode) == false)
	{
		UE_LOG(LogBlueshiftImageQuality, Warning, TEXT("%hs: Upscaler %s is not supported"),
			__FUNCTION__,
	...
	// Log
	UE_LOG(LogBlueshiftImageQuality, Log, TEXT("%hs: Desired upscaler changed from %s to %s"),
		__FUNCTION__,
...
bool UBlueshiftIQManager::IsUpscalerSupported(const EBlueshiftUpscalerMode InUpscalerMode) const
{
...
TArray<EBlueshiftUpscalerMode> UBlueshiftIQManager::GetSupportedUpscalers() const
{
