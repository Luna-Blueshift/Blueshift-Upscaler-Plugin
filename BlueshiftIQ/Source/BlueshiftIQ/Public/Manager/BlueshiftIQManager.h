...
#include "Types/BlueshiftUpscalerMode.h"

#include "BlueshiftIQManager.generated.h"

UCLASS(Config = "GameUserSettings", ConfigDoNotCheckDefaults, BlueprintType, meta = (DisplayName = "Image Quality Manager (Blueshift)"))
class BLUESHIFTIQ_API UBlueshiftIQManager : public UEngineSubsystem
{
	GENERATED_BODY()

public:
	static UBlueshiftIQManager* Get();
...
public:
	...
	//-----------------------------------------------------------------------------------------------------------------
private:
	void ApplyAASettings();
	void SetAAToDefaults();
	 ...
	 */
	UPROPERTY(Config)
	EBlueshiftAAMethod AAMethod = EBlueshiftAAMethod::TAA;

public:
	UFUNCTION(BlueprintPure, Category = "AA")
	EBlueshiftAAMethod GetAAMethod() const { return AAMethod; }

	UFUNCTION(BlueprintCallable, Category = "AA")
	void SetAAMethod(const EBlueshiftAAMethod InAAMethod);

	UFUNCTION(BlueprintCallable, Category = "AA")
	bool IsAAMethodSupported(const EBlueshiftAAMethod InAAMethod) const;

	/** Return a list of all the supported AA methods. */
	UFUNCTION(BlueprintCallable, BlueprintPure = "false", Category = "AA")
	TArray<EBlueshiftAAMethod> GetSupportedAAMethods() const;
};