...
bool UBlueshiftUpscalerSubsystem::IsDLSSSupported() const
{
#if WITH_DLSS
...
void UBlueshiftUpscalerSubsystem::UpdateDLSSEnabled()
{
#if WITH_DLSS
...
void UBlueshiftUpscalerSubsystem::ApplyDLSSSettings()
{
#if WITH_DLSS
	check(IsInGameThread());
	...
	if (bIsSupported)
	{
		...
		UE_LOG(LogBlueshiftUpscaler, Log, TEXT("%hs: DLSS changed r.ScreenPercentage from %f to %f (desired: %f)"), __FUNCTION__, OldScreenPercentage, CVarScreenPercentage->GetFloat(), OptimalScreenPercentage);
	}
	// Warn
	else
	{
		UE_LOG(LogBlueshiftUpscaler, Warning, TEXT("%hs: DLSS Mode %s is not supported"), __FUNCTION__, *UEnum::GetDisplayValueAsText(DLSSPluginMode).ToString());
	}
...
void UBlueshiftUpscalerSubsystem::SetDLSSToDefaults()
{
#if WITH_DLSS
...
void UBlueshiftUpscalerSubsystem::FixDLSSMode()
{
#if WITH_DLSS
...
void UBlueshiftUpscalerSubsystem::SetDLSSMode(int32 InDLSSMode)
{
	DLSSMode = InDLSSMode;
	FixDLSSMode();
}

void UBlueshiftUpscalerSubsystem::SetDLSSRREnabled(const bool bInEnabled)
{
#if WITH_DLSS
	if (bInEnabled
...
bool UBlueshiftUpscalerSubsystem::IsDLSSRRSupported() const
{
#if WITH_DLSS
